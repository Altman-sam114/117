...
v0.86 在 `MarkdownTextInputConfigurationTests` 相对 v0.85 新增 7 项同步状态纯值与 Coordinator 接线测试：初始状态进入外部正文/选区同步；正文实际发布后记录正文与选区 token，并只对正文和选区都匹配的非 marked bridge 更新消费一次快速路径；`textViewDidChangeSelection` 只更新选区、不建立正文快速路径；外部正文即使使用相同 `NSRange` 也会回到完整同步；外部选区不匹配会回到完整同步；marked text 会延迟且不消费本地发布状态；`textViewDidChange` 会记录下一次 bridge 更新的本地发布状态。回车续写、缩进和既有 BindingProbe 契约继续保留，测试不创建 SwiftUI host、不启动 App、不使用真实键盘或计时断言。实现阶段只允许 `git diff --check`、cached diff check、Swift parse、workflow YAML 和必要 `rg`；本机不运行 build、XCTest、`xcodebuild`、Simulator、Mac Catalyst App、UI 自动化、截图、性能脚本或 Instruments。v0.85 云端基线为 `205 passed / 0 failed / 0 skipped`，v0.86 预期总数高于 205；当前云端阶段曾因快速路径 P1 和文档事实不一致退回，尚无可记录的最终 run/artifact，结果待最新实现 HEAD 的 Agent C artifact 核对。
