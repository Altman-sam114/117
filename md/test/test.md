...
- `JournalStoreTests` 在 v0.73 从 5 项重构为 15 项：production writer JSON 字节策略、乱序拒绝、同 revision 幂等、失败重试；starter actor 路径；gate writer 等待期间 MainActor 可运行；手动 scheduler debounce；flush 取消/等待/追赶；create/delete 内存即时与 flush 后磁盘结果；无效删除；旧失败仲裁、当前失败重试、读取错误保留和既有排序。测试不使用固定 sleep、轮询、GCD、semaphore、detached task 或真实大文件猜测时序。预期云端总数为 `178 passed / 0 failed / 0 skipped`，最终以最新 artifact 为准；这些测试不替代 Instruments、真实后台挂起、强杀或断电验证。
