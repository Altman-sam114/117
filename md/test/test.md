...
- 当前没有第三方依赖和包管理器。
- 当前已有 `MDJournalTests` XCTest target，覆盖核心模型、非持久化正文派生、列表派生快照、Markdown 解析、统计、Markdown snippet 和 `JournalStore` 写入节流。
...
```text
mdjournal-ci-v0.13-main-<short_sha>-run<run_id>-attempt<run_attempt>
```
