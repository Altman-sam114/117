...
- 当前没有第三方依赖和包管理器。
- 当前已有 `MDJournalTests` XCTest target，覆盖核心模型、非持久化正文派生、列表派生快照、Markdown 解析、统计、Markdown snippet、Markdown 片段插入规则、写作命令快捷键和 `JournalStore` 写入节流。
...
- 修改 `JournalEntry`、`JournalSection`、`MarkdownBlockParser`、`JournalStatistics`、`MarkdownSnippet`、`MarkdownSnippetInsertion` 或 `EditorWritingCommand` 等已有 XCTest 覆盖的核心规则。
...
- 修改 `JournalStore` 加载、创建、更新、删除、保存、节流或 flush 行为。
...
```text
mdjournal-ci-v0.15-main-<short_sha>-run<run_id>-attempt<run_attempt>
```
...
- 正文工具栏、Mac Catalyst “插入 Markdown”菜单和键盘快捷键能在当前光标处插入片段。
- 选中文本后，加粗、斜体、代码块能包裹选区，引用、列表和待办能逐行转换选区。
- 搜索和分类筛选可用。
- 统计看板指标随数据变化。
- iPhone 竖屏和横屏布局不重叠。
