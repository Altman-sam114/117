...
v0.84 摘要共享正文扫描专项核对（实现 HEAD artifact 待生成）：
...
- Agent C 必须只验收最新 `origin/main` 对应的 v0.84 run/artifact，核对 manifest 的 `version`、完整 `commitSha`、`branch=main`、`runId`、`runAttempt`、workflow、结果路径和 static/iOS/Mac Catalyst/XCTest outcome；JUnit 的 failures/errors/skipped 必须为 0，XCTest 必须高于 v0.83 的 203 项且新增摘要 characterization 实际执行并通过。
- Agent C 还必须核对 static/build/Catalyst/test 日志、failure summary、三份 `.xcresult` 及 `Info.plist`/summary，以及未加密 ZIP 的 GitHub/local size、digest、CRC、fresh extract、清单和逐文件 SHA-256；本条在 artifact 产生前不填充 run、artifact、digest 或测试结果。
- 阶段一通过后，docs-close HEAD 必须重新触发并下载自己的 artifact；阶段一结果不能冒充最终 HEAD。纯值测试和 CI 仍不能证明真实 Mac Catalyst 长文帧率、分配、IME、VoiceOver 或像素排版。
