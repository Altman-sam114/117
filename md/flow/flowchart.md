...
flowchart TD
  ...
  BodyTextView --> LineIndentation["MarkdownLineIndentation：当前行或多行选区缩进/反缩进，扫描到选区有效结束行并收集 UTF-16 offset，删除 tab 或最多两个行首空格"]
...
flowchart LR
  ...
  ContinueRule --> Body
  BodyText --> IndentRule["MarkdownLineIndentation：Tab / Shift-Tab 行缩进，扫描到选区有效结束行并收集 UTF-16 offset，反缩进删除 tab 或最多两个行首空格"]
  IndentRule --> Body
