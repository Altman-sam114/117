...
- 列表筛选、分类计数、编辑器头部和统计看板复用非持久化派生结果；统计和列表概览使用轻量正文 metrics，避免不需要摘要时生成 excerpt，并预计算分布条最大值、主导分类/心情和最近 7 天趋势最大词数。
- 编辑器头部直接使用轻量正文 metrics 展示词数和 `###` 小节，避免正文输入时为未展示的摘要生成 excerpt。
...
- Mac Catalyst 下保留列表、编辑器、预览和统计主流程，并补充右键删除、“日记”菜单、“写作”菜单、`⌘N` 新建、独立统计窗口、Markdown 片段菜单、写作工具栏入口、光标/选区片段插入和可见缩进/反缩进入口。
