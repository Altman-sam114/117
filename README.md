...
- Markdown 编辑器，带 `###` 小节、加粗、引用、列表、待办、代码块和分割线快捷按钮；Mac Catalyst 下也可从“插入 Markdown”菜单、写作工具栏和键盘快捷键追加片段。
...
- Mac Catalyst 下保留列表、编辑器、预览和统计主流程，并补充右键删除、“日记”菜单、“写作”菜单、`⌘N` 新建、独立统计窗口、Markdown 片段菜单和写作工具栏入口。
...
当前已建立 `MDJournalTests` 单元测试 target，覆盖核心模型、列表派生快照、Markdown 解析、统计、Markdown 快捷片段、写作命令快捷键和 `JournalStore` 写入节流。需要本机尝试 XCTest 时使用：
