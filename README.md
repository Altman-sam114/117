...
- 2026-08-23（v0.84 实现阶段）：列表摘要把 metrics 与完整正文 excerpt 合并到同一次 `JournalBodyDerivation` 扫描，新增纯值 characterization 覆盖摘要 Markdown 口径、换行、空标题、EOF、fenced code、Unicode/组合字符和长正文；`JournalSection.excerpt` 仍保留独立的 task/list marker 清理路径。Agent B 本轮只执行轻量检查并已将 CI 版本更新为 `v0.84`；本条记录暂不填充云端 run、artifact 或测试数量，完整 build/XCTest 与结果包完整性待 Agent C 针对最新 `origin/main` 复核。
