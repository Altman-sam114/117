...
- Markdown 正文支持 Tab / Shift-Tab 对当前行或多行选区缩进和反缩进，便于 Mac Catalyst 与硬件键盘写作。
...
当前已建立 `MDJournalTests` 单元测试 target，覆盖核心模型、正文 summary / metrics 派生一致性、列表派生快照、列表概览轻量统计快照、Markdown 解析（含有序列表块和 `###` 小节分组）、统计、Markdown 快捷片段、片段插入规则（含选区空白行跳过）、无序列表/待办/引用/有序列表回车续写规则、Markdown 行缩进规则、Markdown 输入配置、写作命令快捷键、`JournalStore` 写入节流和更新按需排序。需要本机尝试 XCTest 时使用：
