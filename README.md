...
- Markdown 列表、待办和引用支持回车续写；非空项会延续同缩进前缀，空项按回车退出当前结构。
...
当前已建立 `MDJournalTests` 单元测试 target，覆盖核心模型、列表派生快照、列表概览轻量统计快照、Markdown 解析、统计、Markdown 快捷片段、片段插入规则、列表/待办/引用回车续写规则、Markdown 行缩进规则、Markdown 输入配置、写作命令快捷键、`JournalStore` 写入节流和更新按需排序。需要本机尝试 XCTest 时使用：
