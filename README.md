...
- Markdown 预览在单次渲染中复用同一份解析结果和小节分组判断；解析器空行判断直接扫描水平空白，不创建临时 trimmed 字符串；内联文本没有 Markdown 触发字符时直接走纯文本 `AttributedString`，并用索引迭代渲染块和列表项，减少大正文编辑时的重复解析、重复派生和临时数组分配。
