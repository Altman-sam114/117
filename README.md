...
- 列表筛选、分类计数、编辑器头部和统计看板复用非持久化派生结果；统计和列表概览使用轻量正文 metrics，避免不需要摘要时生成 excerpt，并预计算分布条最大值和最近 7 天趋势最大词数。
...
- Markdown 正文支持 Tab / Shift-Tab 对当前行或多行选区缩进和反缩进；反缩进会删除一个 tab 或最多两个行首空格，便于 Mac Catalyst 与硬件键盘写作。
...
- Markdown 预览在单次渲染中复用同一份解析结果，减少大正文编辑时的重复解析。
...
- Mac Catalyst 下保留列表、编辑器、预览和统计主流程，并补充右键删除、“日记”菜单、“写作”菜单、`⌘N` 新建、独立统计窗口、Markdown 片段菜单、写作工具栏入口和光标/选区片段插入。
- 宽屏下编辑器使用左右分栏，同时展示编辑区和 Markdown 预览；Mac Catalyst 写作工具栏可隐藏或显示预览栏，给正文更宽的写作空间。
...
当前已建立 `MDJournalTests` 单元测试 target，覆盖核心模型、正文 summary / metrics 派生一致性、列表派生快照、列表概览轻量统计快照、Markdown 解析（含有序列表块和 `###` 小节分组）、统计、Markdown 快捷片段、片段插入规则（含选区空白行跳过）、无序列表/待办/引用/有序列表回车续写规则、Markdown 行缩进规则（含单空格反缩进）、Markdown 输入配置、写作命令快捷键、`JournalStore` 写入节流和更新按需排序。需要本机尝试 XCTest 时使用：
